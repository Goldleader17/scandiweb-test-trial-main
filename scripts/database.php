<?php
    ...
    class DatabaseActivity {
        ...
        private function checkIfSkuIsTaken($sku) {
            ...
            $checkForSkuSql = $this->databaseHandle->prepare('SELECT * FROM products WHERE product_sku = ?');
        ...
        public function outputAllSkuTakenValues() {
            // Preparing and executing SQL query for getting product based on the SKU value.
            $query = $this->databaseHandle->prepare('SELECT product_sku FROM products');
            $query->execute();
        ...
        public function addProductToDatabase() {
            ...
            $query = 'INSERT INTO products (product_sku, product_name, product_price, product_type, product_size, product_weight, product_height, product_width, product_length) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
        ...
        public function displayProductsFromDatabase() {
            ...
            $sql = $this->databaseHandle->prepare('SELECT * FROM products');
        ...
        public function deleteCheckedProductsFromDatabase() {
            ...
            if (isset($_POST['deleteCheckbox'])) {
                ...
                $sql = $this->databaseHandle->prepare('DELETE FROM products WHERE product_sku = ?');
